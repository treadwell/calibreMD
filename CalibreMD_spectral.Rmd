...
prep_encoder_data <- function(eav, unified_meta, binary_embedding = NULL, indexed_features = NULL) {
  # deps: dplyr, tidyr, purrr, Matrix
  
  # 1) Book node_ids in a stable order
  ids <- unified_meta %>%
    dplyr::filter(type == "book") %>%
    dplyr::arrange(.data$node_id) %>%
    dplyr::pull(.data$node_id)
  
  # 2) Align Y (binary_embedding) to ids (rows = ids). Robust to missing rownames/ids.
  Y <- NULL
  if (!is.null(binary_embedding)) {
    # If rownames exist, use them; otherwise assume rows are already in node_id order
    if (!is.null(rownames(binary_embedding))) {
      rix <- match(ids, rownames(binary_embedding))
      Y <- binary_embedding[rix, , drop = FALSE]
    } else {
      # best effort: index by integer positions if dimensions match
      if (nrow(binary_embedding) >= length(ids)) {
        Y <- binary_embedding[seq_along(ids), , drop = FALSE]
      } else {
        stop("binary_embedding has no rownames and too few rows to align with ids.")
      }
    }
  }
  
  # 3) Feature rows (book_id, feature, value) restricted to book ids and de-duped
  #    Assumptions:
  #      - eav has columns: book_id, feature, value
  #      - unified_meta has node_id (book nodes) and type
  #    If your eav uses 'id' instead of 'book_id', change 'book_id' below to 'id'.
  book_features <- eav %>%
    dplyr::inner_join(
      unified_meta %>% dplyr::filter(type == "book") %>% dplyr::select(book_id, node_id),
      by = dplyr::join_by(book_id == book_id)
    ) %>%
    dplyr::filter(!.data$feature %in% c("tag", "title_original")) %>%
    dplyr::select(node_id, feature, value) %>%
    dplyr::distinct()
  
  # 4) Create (or reuse) the enumerated feature-value dictionary
  #    Columns: feature, value, key, id_feature
  if (is.null(indexed_features)) {
    ...
    req_cols <- c("feature", "value", "key", "id_feature")
    if (!all(req_cols %in% colnames(indexed_features))) {
      stop("indexed_features must contain columns: feature, value, key, id_feature")
    }
  }
  
  # 5) Build sparse X: rows = ids (node_id order), cols = id_feature (feature=value)
  #    One nonzero (1) per (book_id, feature=value) pair.
  #    Map book_id -> row index, key -> column index
  ...
  if (!requireNamespace("Matrix", quietly = TRUE)) {
    stop("Package 'Matrix' is required to build the sparse matrix X.")
  }
  ...
  # 6) Return IDs, dictionary, X, and Y
  list(
    ids = ids,
    features = indexed_features,  # dictionary of columns in X
    X = X,                        # sparse one-hot feature matrix (ids x feature=value)
    Y = Y                         # aligned target/embedding (optional)
  )
}

### STOP  TEST THIS!  FIX Y, review all comments
```
