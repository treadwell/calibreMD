...
positive_edges <- book_tags %>% 
  left_join(book_ids,  by = "book_id")  %>%   # adds column `node_id`
  rename(node_a = node_id) %>%                # book → node_a
  left_join(tag_names, by = "tag_name") %>%   # adds 2nd `node_id`
...
# ---- 2. Make it bidirectional (A→B and B→A)
symmetric_edges <- bind_rows(
  positive_edges,
  transmute(positive_edges, node_a = node_b, node_b = node_a)
)

g <- igraph::graph_from_edgelist(symmetric_edges %>% select(node_a, node_b) |> as.matrix(), directed = FALSE)
components(g)$no

#TODO
# igraph on number of components.
# send .db file to Matt

# Now we need to ensure the graph is fully connected:
# Option 0: semantically meaningful tag similarity (unnesting tags), connecting hierarchies
#    0.1 maybe unnest at eav stage?
# Option 1: randomly sample negative edges between books that share no tags 
# Option 2: Add feature-based MST positive and negative edges (per tag set positive, intra-tag set negative)
...
# Check if the problem is in the star graph construction
# cat("\n=== STAR GRAPH ANALYSIS ===\n")
# cat("Number of tags:", nrow(tag_membership), "\n")
# cat("Number of star graph edges:", nrow(star_graph_pairs), "\n")
# cat("Number of unique books in star graph:", length(unique(c(star_graph_pairs$from, star_graph_pairs$to))), "\n")
...
get_nearest_neighbors <- function(binary_embedding, book_idx, num_neighbors) {
  ...
  nearest_ids <- purrr::map(nearest_indices, \(i) nodes[i])
  list(ids = nearest_ids, indices = nearest_indices, distances = nearest_distances)
}
...
test_book_idx <- node_index[test_book_id]
...
if (length(test_book_idx) > 0) {
  ...
  # Get tags for neighbors
  neighbor_tags <- eav %>% 
    filter(feature == "tag", id %in% as.numeric(neighbors$ids)) %>%
    group_by(id) %>%
    summarize(tags = paste(value, collapse = ", "), .groups = "drop")
  
  cat("\nNeighbor tags:\n")
  for (i in 1:nrow(neighbor_tags)) {
    cat("Book", neighbor_tags$id[i], "(distance:", round(neighbors$distances[i], 3), "):", neighbor_tags$tags[i], "\n")
  }
...
get_neighbor_tags <- function(existing_tags, neighbor_indices, binary_embedding) {
  ...
  for (i in seq_along(neighbor_indices)) {
    neighbor_id <- rownames(binary_embedding)[neighbor_indices[i]]
    if (is.null(neighbor_id)) neighbor_id <- paste0("book_", neighbor_indices[i])
...
## Main: Recommend new tags using spectral analysis (refactored)
...
add_new_tags_spectral <- function(eav, binary_embedding, 
  ...
  for (book_idx in 1:n_books) {
    ...
    nn <- get_nearest_neighbors(binary_embedding, book_idx, min_neighbors)
    all_neighbor_tags <- get_neighbor_tags(existing_tags, nn$indices, binary_embedding)
    high_score_tags <- score_tags(all_neighbor_tags, nn$distances, book_existing_tags, threshold, max_recommendations)
    if (!is.null(high_score_tags)) {
      recommendations[[book_id]] <- list(
        ...
        nearest_neighbors = rownames(binary_embedding)[nn$indices],
        neighbor_distances = nn$distances
      )
    }
  }
