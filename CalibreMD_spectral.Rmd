...
## FN: Prep data set
```{r}
prep_encoder_data <- function(eav, unified_meta, binary_embedding){
  
  # produce filtered list of node_ids corresponding to books
  ...
  # filter the binary embeddings to include the node_ids in that order (Y)
  
  Y <- if(is.null(binary_embedding)) {
    NULL
  } else {
    binary_embedding[ids, , drop = FALSE]
  }
  
  # generate a sparse matrix of the observable features for each node_id (X)

  book_features <- eav %>% 
    inner_join(unified_meta, by = c("id", "book_id")) %>% 
    filter(node_id %in% ids,
           !feature %in% c("tag", "title_original")) %>% 
    mutate(feature = paste(feature, value, sep = ": ")) %>% 
    select(id, feature) %>% 
    distinct() %>% 
    arrange(id)
  
  #### STOPPED Here  ### construct X
