...
#dataDir <- '/Users/kbrooks/Dropbox/Books/calibreGPT_test_lg'
...
```{r}
binarize <- function(X, n_iter = 25, mode = NULL) {
  if (is.null(mode) || mode == "itq") {
    # Step 1: Zero-center
...
binary_embedding_train <- binarize(eigvecs)  # uint8-like 0/1 integer matrix
```
...
recommend_tags <- function(margin,
  ...
  for (i in seq_along(book_row_ids)) {
    ...
    if (!is.null(distance_cap)) {
      df_tags_filtered <- df_tags |>
        dplyr::filter(.data$distance <= distance_cap)
      if (nrow(df_tags_filtered) == 0) {
        df_tags_filtered <- df_tags %>% dplyr::slice_head(n = 1)
      }
      df_tags <- df_tags_filtered
    }
  
    results_rows[[length(results_rows) + 1L]] <- tibble::tibble(
      book_id  = as.character(this_book_id),
      tag_name = as.character(df_tags$tag_name),
      distance = as.numeric(df_tags$distance)
    )
