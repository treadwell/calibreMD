...
# Set row names of binary_embedding to match dataset$X
# Only use the first nrow(dataset$X) rows of binary_embedding
if (!is.null(dataset$X)) {
  n_dataset_books <- nrow(dataset$X)
  if (nrow(binary_embedding) >= n_dataset_books) {
    # Take only the first n_dataset_books rows
    binary_embedding <- binary_embedding[1:n_dataset_books, , drop = FALSE]
    rownames(binary_embedding) <- rownames(dataset$X)
    cat("Aligned binary_embedding to dataset: ", nrow(binary_embedding), "x", ncol(binary_embedding), "\n")
  } else {
    cat("Warning: binary_embedding has fewer rows than dataset$X\n")
  }
}
...
binary_classifiers <- train_binary_classifiers(dataset, binary_embedding)
...
# Run the tests
test_results <- test_binary_classifiers(binary_classifiers, dataset, binary_embedding)
```
...
test_books <- dataset$X[1:5, , drop = FALSE]  # First 5 books
encoded_test_books <- encode_new_books(binary_classifiers, test_books)
...
add_new_tags_spectral <- function(eav, binary_classifiers, dataset, binary_embedding, 
  ...
  # Debug: Check for tag name issues
  cat("Debug: Checking tag names...\n")
  sample_tags <- eav %>% 
    filter(feature == "tag") %>% 
    pull(value) %>% 
    head(10)
  cat("Sample tags:", paste(sample_tags, collapse = ", "), "\n")
  
  # Debug: Check EAV structure for tags
  cat("Debug: Checking EAV structure for tags...\n")
  tag_structure <- eav %>% 
    filter(feature == "tag") %>% 
  ...
  cat("Debug: Checking for duplicate tag entries...\n")
  duplicate_check <- eav %>% 
    filter(feature == "tag") %>% 
    group_by(id, value) %>% 
  ...
  if (nrow(duplicate_check) > 0) {
    cat("Found duplicate tag entries:\n")
    print(duplicate_check)
  } else {
    cat("No duplicate tag entries found.\n")
  }
  
  # Debug: Check for tags with dots or spaces
  cat("Debug: Checking for tags with dots or spaces...\n")
  tags_with_dots <- eav %>% 
    filter(feature == "tag") %>% 
    filter(grepl("\\.", value)) %>% 
    pull(value) %>% 
    unique() %>% 
    head(5)
  cat("Tags with dots:", paste(tags_with_dots, collapse = ", "), "\n")
  
  tags_with_spaces <- eav %>% 
    filter(feature == "tag") %>% 
    filter(grepl(" ", value)) %>% 
    pull(value) %>% 
    unique() %>% 
    head(5)
  cat("Tags with spaces:", paste(tags_with_spaces, collapse = ", "), "\n")
  
  # Debug: Check book IDs
  cat("Debug: Checking book IDs...\n")
  sample_book_ids <- rownames(all_books_encoded)[1:5]
  cat("Sample book IDs:", paste(sample_book_ids, collapse = ", "), "\n")
  
  # Debug: Check dataset row names
  cat("Debug: Dataset row names (first 5):", paste(head(rownames(dataset$X), 5), collapse = ", "), "\n")
  cat("Debug: Binary embedding row names (first 5):", paste(head(rownames(binary_embedding), 5), collapse = ", "), "\n")
  
  # Debug: Check existing tags structure
  cat("Debug: Checking existing tags structure...\n")
  sample_existing <- existing_tags %>% head(3)
  print(sample_existing)
  ...
  for (book_idx in 1:n_books) {
    ...
    for (i in seq_along(nearest_indices)) {
      ...
      if (nrow(neighbor_existing) > 0) {
        ...
        if (book_idx <= 2 && i == 1) {
          cat("Debug: Neighbor", neighbor_id, "tags:", paste(neighbor_tags[[i]], collapse = ", "), "\n")
        }
      } else {
    ...
    if (length(all_neighbor_tags) > 0) {
      ...
      # Debug: Check raw tag names before counting
      if (book_idx <= 3) {
        cat("Book", book_id, "raw neighbor tags:", paste(all_neighbor_tags, collapse = ", "), "\n")
      }
      ...
      # Debug: Check tag names in counts
      if (book_idx <= 3) {  # Only debug first few books
        cat("Book", book_id, "neighbor tags:", paste(names(tag_counts), collapse = ", "), "\n")
        cat("Book", book_id, "tag counts:", paste(tag_counts, collapse = ", "), "\n")
      }
      
      # Debug: Check for any tag names that look duplicated
      suspicious_tags <- names(tag_counts)[grepl("\\..*\\..*", names(tag_counts))]
      if (length(suspicious_tags) > 0) {
        cat("Debug: Found suspicious tag names:", paste(suspicious_tags, collapse = ", "), "\n")
      }
      ...
      # Debug: Check tag_scores names
      if (book_idx <= 3) {
        cat("Debug: Book", book_id, "tag_scores names:", paste(names(tag_scores), collapse = ", "), "\n")
        cat("Debug: Book", book_id, "tag_scores values:", paste(tag_scores, collapse = ", "), "\n")
      }
      ...
      if (length(new_tags) > 0) {
        ...
        # Debug: Check tag names before threshold filtering
        if (book_idx <= 3) {
          cat("Debug: Book", book_id, "new tags before threshold:", paste(names(new_tag_scores), collapse = ", "), "\n")
        }
        ...
        # Debug: Check tag names after threshold filtering
        if (book_idx <= 3) {
          cat("Debug: Book", book_id, "high score tags:", paste(names(high_score_tags), collapse = ", "), "\n")
        }
        
        if (length(high_score_tags) > 0) {
          # Debug: Check what's being stored
          if (book_idx <= 2) {
            cat("Debug: Book", book_id, "recommended tags:", paste(names(high_score_tags), collapse = ", "), "\n")
          }
  ...
  if (length(recommendations) > 0) {
    ...
    # Debug: Check final tag names
    cat("Debug: Final tag names in results:\n")
    print(unique(results$recommended_tag))
