...
```{r}
# Assumptions (from your workspace):
# - book_features has columns: id, feature, row_index, col_index (from training-time mapping)
# - feature_levels: vector of all features seen during training
# - predict_ids: vector of book IDs, used for row indexing
# - models: a list of trained XGBoost models, each with $feature_names
# - You want to produce: pred_probs_new = data.frame(book_id, <tag1>, <tag2>, ...)
...
```{r}
#TODO: don't add tags for which a more detailed tag already exists, e.g., if "Computer Science.SD&E" exists, don't recommend "Computer Science"
...
```{r}
# Step 1: Join actual tags with predicted probabilities
joined_tags <- book_tags %>%
  rename(book_id = id, tag = tag) %>%
  left_join(pred_long, by = c("book_id", "tag"))

# Step 2: Filter where tag exists but model assigns low probability (or is NA = no prediction)
questionable_tags_df <- joined_tags %>%
  filter(is.na(prob) | prob < 0.05) %>%
  group_by(book_id) %>%
  summarize(tags_to_review = paste(tag, collapse = ", "), .groups = "drop")

# Step 3: Add titles from EAV
book_titles_df <- result %>%
  filter(feature == "title") %>%
  select(book_id = id, title = value) %>%
  distinct()

# Step 4: Join for final output
final_removal_recommendations <- questionable_tags_df %>%
  left_join(book_titles_df, by = "book_id") %>%
