...
```{r}
# Recreate book_features without filtering
book_features <- eav %>% 
  filter(feature != "tag") %>%                                 # exclude tag rows
  mutate(feature = paste(feature, value, sep = ": ")) %>%      # combine into feature: value
  select(id, feature) %>% 
  distinct() %>%
  mutate(
    row_index = match(id, predict_ids),                        # map to row indices
    col_index = match(feature, feature_levels)                 # map to training-time feature space
  ) %>%
  filter(!is.na(row_index) & !is.na(col_index)) %>%            # remove invalid features or ids
  arrange(row_index)
