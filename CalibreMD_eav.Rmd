...
```{r}
# Step 1: Normalize tags (defensive coding)
book_tags_clean <- book_tags %>%
  mutate(tag = str_trim(str_to_lower(tag))) %>%
  rename(book_id = id)

pred_long_clean <- pred_long %>%
  mutate(tag = str_trim(str_to_lower(tag)))

# Step 2: Join actual tags with model predictions
joined_tags <- book_tags_clean %>%
  inner_join(pred_long_clean, by = c("book_id", "tag"))

# Step 3: Filter for tags the model says are very unlikely
